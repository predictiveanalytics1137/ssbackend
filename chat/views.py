...
from rest_framework import status
from django.contrib.auth.models import User
from .models import ChatFileInfo, FileSchema
from django.core.exceptions import ObjectDoesNotExist
import logging

logger = logging.getLogger(__name__)

class TrainingSchemaView(APIView):
    """
    Returns the 'training' dataset's schema for a given user & chat,
    so the front-end can compare it to the user's new CSV before uploading.
    """
    def get(self, request, user_id, chat_id):
        """
        GET /api/training_schema/<user_id>/<chat_id>/
        
        Returns JSON like:
        {
          "columns": [
            {"column_name": "store", "data_type": "int"},
        ...
        # 2. Retrieve the earliest (or some specific) ChatFileInfo for the training dataset
        #    e.g. the earliest uploaded file for that chat
        try:
            # For example: get the first ChatFileInfo for this user/chat
            # If you have an is_training flag, filter for that.
            chat_file = ChatFileInfo.objects.filter(
                user=user,
                chat__chat_id=chat_id
            ).order_by('created_at').first()

            if not chat_file:
                return Response({"error": "No training file found for this chat."},
                                status=status.HTTP_404_NOT_FOUND)

            # 3. Access the FileSchema
            #    Each ChatFileInfo has a 'file' -> an UploadedFile -> a one-to-one 'schema'
            if not hasattr(chat_file.file, 'schema'):
                return Response({"error": "No schema found for the training file."},
                                status=status.HTTP_404_NOT_FOUND)

            file_schema_obj = chat_file.file.schema  # This is a FileSchema instance
            # The actual columns are in file_schema_obj.schema => list of {column_name, data_type}

            data = {
                "columns": file_schema_obj.schema
            }
            return Response(data, status=status.HTTP_200_OK)

        except ObjectDoesNotExist:
            return Response({"error": "Training schema not found."},
                            status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            logger.error(f"Error retrieving training schema: {e}", exc_info=True)
            return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
