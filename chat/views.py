...
# #             }, status=status.HTTP_200_OK)
# #         else:
# #             # For non-time-based notebooks, combine the entity/target and features queries in one notebook.
...
# #     # NEW: Combined function for non–time-based notebook creation.
# #     # def create_non_time_based_notebook(self, entity_id_column, target_column, feature_columns, table_name, columns_list):
# #     #     import nbformat
# #     #     from nbformat.v4 import new_notebook, new_markdown_cell, new_code_cell, new_output
# #     #     print("[DEBUG] Creating non-time-based notebook...")
# #     #     nb = new_notebook()
# #     #     cells = []

# #     #     # Step 1: Entity & Target Query
# #     #     cells.append(new_markdown_cell("### Entity & Target Query"))
# #     #     sanitized_entity_id_column = self.sanitize_identifier(entity_id_column) if entity_id_column else "*"
# #     #     sanitized_target_column = self.sanitize_identifier(target_column) if target_column else "*"
# #     #     query_entity_target = f"SELECT {sanitized_entity_id_column}, {sanitized_target_column} FROM {table_name} LIMIT 10;"
# #     #     cells.append(new_code_cell(query_entity_target))

# #     #     # Step 2: Features or Attributes Query
# #     #     cells.append(new_markdown_cell("### Features or Attributes Query"))
# #     #     if feature_columns:
# #     #         sanitized_features = [self.sanitize_identifier(feature) for feature in feature_columns]
...
# #     #             cells.append(new_markdown_cell(f"**Error:** {error_message}"))
# #     #         else:
# #     #             query_features = f"SELECT\n    " + ",\n    ".join(sanitized_features) + f"\nFROM {table_name}\nLIMIT 10;"
# #     #             cells.append(new_code_cell(query_features))
# #     #     else:
# #     #         cells.append(new_markdown_cell("No feature columns specified."))
...
# #         # Sanitize columns; use "*" as fallback if empty.
...
# #         # Add the code cell for the entity-target query if it is non-empty.
...
# #         filtered_cells = []
# #         for cell in cells:
# #             # The source can be a string or a list; join if needed.
...
# #         # --- Remove duplicate cells based on their source content ---
# #         unique_cells = []
# #         seen_sources = set()
...
# # In-memory dictionaries to track confirmations and pending updates.
# awaiting_confirmation = defaultdict(bool)
# pending_column_updates = defaultdict(dict)
...
# # --- Updated infer_column_dtype ---
# from dateutil import parser as dateutil_parser

# def infer_column_dtype(series: pd.Series, threshold: float = 0.8) -> str:
#     """
#     Dynamically infer the data type of a pandas Series.
    
#     - If all values are digits, attempts to parse them as a date (e.g. YYYYMMDD or YYMMDD).
#     - Otherwise, tries numeric conversion first. If a sufficient ratio converts, returns "int", "bigint", or "double".
#     - Then, if at least 95% of the (lowercased) non-null values belong to an allowed boolean set,
#       the column is marked as "boolean".
#     - Only if numeric and boolean tests fail does it try date parsing.
#     - If none pass the threshold, returns "string".
#     """
#     series_clean = series.dropna().astype(str).str.strip()
#     if series_clean.empty:
#         return "string"
#     total_count = len(series_clean)
    
#     def parse_ratio(parsed_series):
#         return parsed_series.notnull().sum() / float(total_count)
    
#     # 1. If all values are digits, try to detect a date format (e.g. YYYYMMDD or YYMMDD)
#     if series_clean.str.isdigit().all():
#         lengths = series_clean.str.len()
#         mode_length = lengths.mode()[0]
#         if mode_length == 8:
#             dt_series = pd.to_datetime(series_clean, format="%Y%m%d", errors='coerce')
#             if parse_ratio(dt_series) >= threshold:
#                 return "timestamp"
#         if mode_length == 6:
#             dt_series = pd.to_datetime(series_clean, format="%y%m%d", errors='coerce')
#             if parse_ratio(dt_series) >= threshold:
...
#         ratio_numeric = numeric_series.notnull().sum() / float(total_count)
#         if ratio_numeric >= threshold:
#             if (numeric_series.dropna() % 1 == 0).all():
#                 int_min = numeric_series.min()
#                 int_max = numeric_series.max()
#                 if int_min >= -2147483648 and int_max <= 2147483647:
#                     return "int"
#                 else:
#                     return "bigint"
...
#     # 3. Check for boolean values: if at least 95% are in the allowed set.
#     lower_series = series_clean.str.lower()
#     allowed_booleans = {"true", "false", "1", "0", "yes", "no", "t", "f", "y", "n"}
#     bool_ratio = lower_series.isin(allowed_booleans).mean()
#     if bool_ratio >= 0.95:
#         return "boolean"
    
#     # 4. Try date parsing using pd.to_datetime (with both dayfirst options)
#     for dayfirst_val in [False, True]:
#         dt_series = pd.to_datetime(series_clean, dayfirst=dayfirst_val, errors='coerce')
#         if parse_ratio(dt_series) >= threshold:
#             return "timestamp"
    
#     # 5. Try known date formats explicitly
#     known_formats = ["%Y-%m-%d", "%d-%m-%Y", "%m/%d/%Y", "%d/%m/%Y"]
#     for fmt in known_formats:
#         dt_series = pd.to_datetime(series_clean, format=fmt, errors='coerce')
#         if parse_ratio(dt_series) >= threshold:
#             return "timestamp"
    
#     # 6. As a last resort, try dateutil.parser on each value
#     parsed_list = []
#     for val in series_clean:
#         try:
#             parsed_list.append(dateutil_parser.parse(val))
#         except Exception:
#             parsed_list.append(pd.NaT)
#     dt_series = pd.Series(parsed_list, index=series_clean.index)
#     if parse_ratio(dt_series) >= threshold:
#         return "timestamp"
    
#     return "string"

# # --- Updated standardize_datetime_columns ---
# def standardize_datetime_columns(df: pd.DataFrame, schema: list) -> pd.DataFrame:
#     """
#     For columns inferred as 'timestamp', convert them to ISO format (YYYY-MM-DD HH:MM:SS).
#     Rows that fail to parse become null, but no rows are dropped.
#     """
#     for colinfo in schema:
#         if colinfo["data_type"] == "timestamp":
#             col_name = colinfo["column_name"]
...
#                 df[col_name] = parsed.dt.strftime("%Y-%m-%d %H:%M:%S")
#                 print(f"[DEBUG] Standardized datetime column: {col_name}")
#             except Exception as e:
#                 print(f"[WARNING] Could not standardize date column '{col_name}': {e}")
#     return df
...
# def update_predictive_settings(ps, parsed_updates, schema_columns):
#     """
#     Update only fields explicitly mentioned in parsed_updates with validation.
#     Also auto-update the predictive_question based on target_column, time_frequency, time_frame, and entity_column.
#     """
#     logger.info(f"[update_predictive_settings] Current PS values: {ps.__dict__}")
#     logger.info(f"[update_predictive_settings] Parsed updates: {parsed_updates}")
...
#     def handle_file_upload(self, request, files):
#         """
#         1) Ingest CSV/XLSX, normalize columns, and infer schema.
#         2) Upload to S3 and update Glue.
#         3) Persist file metadata (schema, suggestions, etc.) with the chat ID.
#         """
#         user_id = request.data.get("user_id", "default_user")
#         try:
#             user = User.objects.get(id=user_id)
...
#             ]
#         cells.append(step2_cell)
#         step3_markdown = new_markdown_cell("### Step 3: Summarize the target measure over the chosen time horizon (partial window filter)")
#         cells.append(step3_markdown)
#         step2_sub = query_step2.strip().rstrip(';')
#         horizon_label = time_horizon.replace(' ', '_')
#         query_step3 = f"""
...
#             ]
#         cells.append(step3_cell)
#         step4_markdown = new_markdown_cell("### Step 4: Join additional features (1-year lookback before analysis_time)")
#         cells.append(step4_markdown)
#         if not extra_features:
#             feature_selects = ""
#         else:
...
# # PredictiveSettings detail view
# from rest_framework.views import APIView
# from rest_framework.response import Response
# from rest_framework import status
# from .models import PredictiveSettings
