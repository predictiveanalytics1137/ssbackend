...
#                         na_values=['NA', 'N/A', ''],
#                         on_bad_lines='error',
#                         # parse_dates=['date', 'Date'],  # Explicitly parse the 'date' column
...
#             try:
#                 # Read CSV/XLSX/XLS without parsing dates initially to check columns
#                 if file.name.lower().endswith('.csv'):
#                     # Read the file without parsing dates to inspect column names
#                     df = pd.read_csv(
#                         file,
#                         low_memory=False,
#                         encoding='utf-8',
...
#                 logger.info("[DEBUG] Old columns -> Normalized columns:")
#                 for oc, nc in zip(df.columns.tolist(), normalized_columns):
#                     logger.info(f"   {oc} -> {nc}")
...
#                 # Dynamically detect date-like columns (case-insensitive)
#                 date_columns = []
#                 possible_date_names = ['date', 'datetime', 'timestamp', 'created_at']  # Add common date column names
#                 for col in df.columns:
#                     try:
#                         # Check if the column name matches any possible date name (case-insensitive)
#                         if any(normalize_column_name(name) == col for name in possible_date_names):
#                             # Attempt to parse the column as a datetime
#                             df[col] = pd.to_datetime(df[col], errors='coerce', format='%d-%m-%Y %H:%M:%S')
#                             if df[col].notna().any():  # If any values were successfully parsed
#                                 date_columns.append(col)
#                                 raw_schema = [
#                                     {"column_name": c["column_name"], "data_type": "timestamp"}
#                                     if c["column_name"] == col else c for c in raw_schema
#                                 ]
#                     except Exception as e:
#                         logger.debug(f"[DEBUG] Column {col} is not a datetime: {e}")

#                 if not date_columns:
#                     logger.warning(f"No date-like columns found in {file.name}. Proceeding without date parsing.")
#                 else:
#                     logger.info(f"[DEBUG] Detected date columns: {date_columns}")

#                 # Re-read the file with parsed date columns if any were detected
#                 if date_columns:
...
#                                 na_values=['NA', 'N/A', ''],
#                                 on_bad_lines='error',
#                                 parse_dates=date_columns,  # Use detected date columns
#                                 date_parser=lambda x: pd.to_datetime(x, format='%d-%m-%Y %H:%M:%S', errors='coerce')
#                             )
#                         else:
#                             df = pd.read_excel(file, engine='openpyxl', parse_dates=date_columns)
#                     except Exception as e:
#                         logger.error(f"[ERROR] Error re-reading file with date columns: {e}")
#                         return Response(
#                             {"error": f"Error re-reading file {file.name} with date columns: {str(e)}"},
#                             status=status.HTTP_400_BAD_REQUEST
#                         )
...
#                 # Re-check final schema after date parsing
#                 final_schema = []
#                 for col in df.columns:
#                     final_schema.append({
#                         "column_name": col,
...
#                 possible_date_cols = date_columns if date_columns else []
...
#             except pd.errors.ParserError as e:
#                 logger.error(f"[ERROR] CSV/XLSX/XLS parsing error for {file.name}: {e}")
#                 return Response(
#                     {"error": f"Parsing error for file {file.name}: {str(e)}"},
#                     status=status.HTTP_400_BAD_REQUEST
#                 )
#             except Exception as e:
#                 logger.error(f"[ERROR] Error reading file {file.name}: {e}")
